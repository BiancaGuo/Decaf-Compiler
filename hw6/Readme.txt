...
cfg.py      Constructs CFG for each function by dividing the function into blocks. Does liveness analysis on th CFG by
            calculating successors,defined, used, In and Out variables in a block.Constructs Register Interference graph
            for the function and allocates registers accordingly. Spilling has been taken care of by assigning register
            in the allocation phase. If a failure occurs at this point, we have not implemented memory spilling of that
            registers content. The register will be displayed as None.

mips.py     Generates Mips instruction accordingly. Calculates the stack pointer position in function calls and create load stores
            accordingly.

            Sample foo.decaf calculates fibnacci


Assumptions:
    only 4 arguments supported for a function call